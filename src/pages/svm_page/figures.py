...
def serve_prediction_plot(
    model, X_train, X_test, y_train, y_test, Z, xx, yy, mesh_step, threshold= 0.5
):
...
def serve_roc_curve(model, X_test, y_test):
    ...
    fig = px.area(
        x=fpr, y=tpr, title=f"ROC Curve (AUC={auc_score:.3f})", labels=dict(x="False Positive Rate", y="True Positive Rate"), width=700, height=500
    )
    return fig

def serve_pie_confusion_matrix(model, X_test, y_test, Z, threshold = 0.5):
    # Compute threshold
    scaled_threshold = threshold * (Z.max() - Z.min()) + Z.min()
    ...
    labels = ["TP", "FN", "FP", "TN"]
    colors = ["#13c6e9", "#3182bd", "#ff916d", "#ff744c"]
